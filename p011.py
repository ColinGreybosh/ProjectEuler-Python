...
grid = [
        [8, 2, 22, 97, 38, 15, 00, 40, 00, 75, 4, 0, 7, 78, 52, 12, 50, 77, 91,
         8],
...
def get_prods_of_adj_nums(start, end):
    delta_i, delta_j = tuple(map(sub, end, start))

    if delta_i == 0 or delta_j == 0:  # straight line
        adj_nums = []
        for i in range(start[0], end[0] + 1):
            for j in range(start[1], end[1] + 1):
                adj_nums.append(grid[i][j])
        return reduce(mul, adj_nums)
    if delta_i == delta_j:            # diagonal line
        adj_nums = []
        for i in range(start[0], end[0] + 1):
            adj_nums.append(grid[i][i])
        return reduce(mul, adj_nums)


def get_adj_nums_from_origin(start, length):
    products = []
    for i in range(0, 2):
        for j in range(0, 2):
            end = (start[0] + length * i, start[1] + length * j)
            if 0 <= end[0] < 20 and 0 <= end[1] < 20:
                products.append(get_prods_of_adj_nums(start, end))
    return products


def get_lines_of_adj_nums_of_n_len(n):
    products = []
    for i in range(0, 20):
        for j in range(0, 20):
            products.append(get_adj_nums_from_origin((i, j), n - 1))
    return products


print(max(list((map(max, get_lines_of_adj_nums_of_n_len(4))))))